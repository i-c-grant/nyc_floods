...
** Data issue
It looks like a bunch of calls may have been dumped onto 9/2/21, which didn't have precipitation but has 3000 calls at the exact same time.

** Precipitation data
*** NOAA
**** rnoaa
rnoaa (R package) is out of date, otherwise would use that. Per GitHub conversation, NOAA changed their API and now rnoaa is broken. Replacement package was planned but is now stalled

**** GHCN
The [[https://www.ncei.noaa.gov/products/land-based-station/global-historical-climatology-network-daily][Global Historical Climatology Network]] is a NOAA product that integrates land station climate data.

I downloaded the full historical record from the [[https://www1.ncdc.noaa.gov/pub/data/ghcn/daily/][NOAA archive]], then will narrow it down in R. 

This dataset should be able to provide coarse precipitation data to get going with the statistical analysis.

***** station info for station in Harlem:
#+begin_quote
US1NYNY0074  40.7969  -73.9330    6.1 NY NEW YORK 8.8 N                              
#+end_quote

*** NYS Mesonet
Requested from https://nysmesonet.org/weather/requestdata
Asked for 5-minute precip data since 2020
They should email at ian.grant19@myhunter.cuny.edu

*** NEXRAD Level III
NCEI site kept giving an error saying service was down
Submitted an order to the archive for just 2010 at [[https://www.ncei.noaa.gov/has/HAS.CheckOrderStatus?hasreqid=HAS012515663&emailadd=ian.grant19@myhunter.cuny.edu][this link]]
Should email link to ian.grant19@myhunter.cuny.edu once ready for download
Progress is slow, but processing: 6% after perhaps 30 minutes.

* <2024-04-26 Fri>
** Downloaded NYC Micronet data
U. of Albany responded, I downloaded from link
Dataset is < 1GB
** Downloaded NEXRAD Level 3
But actually the product I want is Stage IV
** Tried to download Stage IV
*** [[https://water.noaa.gov/resources/downloads/precip/stageIV/2024/01/01/][This page ]]
directly from NOAA archive but appears to only have daily
*** [[https://labs.waterdata.usgs.gov/gdp_web/client/#!catalog/gdp/dataset/56d9862ae4b015c306f7c486][USGS request form]]
doesn't work because of OpenDAP issue
*** [[https://data.eol.ucar.edu/cgi-bin/codiac/fgr_form/id=21.093][EOL NCAR page]]
doesn't let you do any spatial subsetting
but tried with 1 month of data. Estimated size is 138 MB.
this means full dataset should be ~20 GB

Emailed for help.

** EDA findings
- Three complaints appear to respond to precipitation: catch basin clogged, street flooding, and sewer backup
- Overall, vast majority of complaints happen at low precipitation because there are many more dry days
- Sewer backup is the most common complaint by far and is precipitation-responsive
- Around 90,000 calls on days with at least half an inch of rain
- Around 55,000 on days with at least an inch
- Of those, vast majority will be less than 2 in
- But the pickup in calls doesn't really happen until after 2 in
- There are only 20,000 such calls


<2024-04-27 Sat>

** EDA findings
- There are about 30 days with more than 2 inches of precipitation in the last 10 years
- about 200 with more than 1 inch

** Descriptor selection
Selected descriptors as follows based on responsiveness to overall daily precipitation:
...
Culvert Blocked doesn't have a strong trend after 2 in, but does show a difference between 1 in and 2+.

Should probably omit COVID time. Seems like the response was less.

* <2024-04-29 Mon>
** Observations from EDA
- remnants of Hurricane Ida, Early September 2021, has by far the most calls and highest precipitation
- however, this event is unique: it is truly extreme within the dataset, I believe the only event of its kind, and also the calls are somewhat lagged and persist over about a week after the rain event
- perhaps better to start with a more typical event: there is a 3-in event latter in fall of 2021 that has a big, synchronized response

** Additional research questions based on EDA
- What accounts for the big differences in magnitude of response among events in the 2-in range? In other words, what additional covariates could improve the daily preicp vs. daily calls linear model?

** Tasks
*** DONE [3/3]  Make daily maps for days with
**** DONE >= 2 in precip
**** DONE >= 3 in precip
**** DONE no precip but calls

* <2024-05-01 Wed>
*** TODO remake datasets with corrected datetime
*** TODO reorganize code, make util scripts
*** TODO do temporal and spatial EDA on all calls: what are the seasonal / time of day effects?
*** TODO download all Stage IV precip data 2010-2024
