...
## calculate intensity
intensity_flood_calls <- spatstat.geom::intensity(ppp_flood_calls_all)
...
## adaptive intensity
adaptive_flood_calls <- adaptive.density(ppp_flood_calls_all)
...
## Ripley's K
k_fit <- Kest(ppp_flood_calls_all, correction = "good")

plot(k_fit)

my_env <- envelope(ppp_flood_calls_all, Kest, nsim = 5)

ppm_fit <- ppm(ppp_flood_calls_all)
...
ppm_fit_cov_y <- ppm(ppp_flood_calls_all ~ elev + slope + y,
                     covariates = list(elev = im_elev,
                                       slope = im_slope))

ppm_fit_slope_y <- ppm(ppp_flood_calls_all ~ slope + y,
                       covariates = list(slope = im_slope))
